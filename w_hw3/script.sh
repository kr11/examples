...
python train.py $use_GPU --model $test_model --epochs=$epoch_size --batch_size=20 --lr=20 --bptt=35 --nhid=200 --emsize=200 --dropout=0.2 --nlayers=2
#    3. CPU-GRU-mine： 已经做了
# 2. batch size 10分钟
...
# 6. embedding size
python train.py $use_GPU --model $test_model --epochs=$epoch_size --emsize=100
python train.py $use_GPU --model $test_model --epochs=$epoch_size --emsize=300
# 7. dropout
# 8. clip
# 9. layers 
